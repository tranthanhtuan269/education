...
class VideoController extends Controller
{
    ...
    public function sendRemoveVideoRequest(Request $request)
    {
        ...
        if ($video) {
            ...
            //     return response()->json([
	        //         'status' => '201',
	        //         'message' => 'Yêu cầu xóa video bài giảng đang được duyệt!'
	        //     ]);
            // }
            ...
            //     ]);
            // }
            // if($video->state == Config::get('app.video_waiting')){  // Nếu đang chờ duyệt thì có thể xóa luôn
            //     //Xóa luôn video trên server
            ...
            if($video->state == Config::get('app.video_active')){
                // $unit = $video->unit;
                // $course = $unit->course;
                $video->state = Config::get('app.video_waiting_to_delete'); //Đang được admin duyệt để xóa
                $video->save();

                return response()->json([
	                'status' => '200',
	                'message' => 'Yêu cầu xóa video đã được gửi.'
	            ]);
                ...
                // DuongNT // Xóa trong bảng usercourse phần tử đại diện video đã xem
                // $unit = $video->unit;
                // $course = $video->unit->course;
                // $user_roles = $course->userRoles()->get()->all();
                // $user_roles = \array_filter($user_roles, function($user_role){
    ...
    public function rejectEditVideo(Request $request)
    {
        ...
        if ($temp_video) {
            ...
            \App\Helper\Helper::addAlertCustomize($video->unit->course->Lecturers()[0]->user, "Bài giảng " . $video->name . "đã bị hủy yêu cầu sửa", "Xin lỗi bạn! Bài giảng <a href='" . url('/') . "/learning-page/" . $video->unit->course->id . "/lecture/" . $video->id . "'>" . $video->name . "</a> đã bị hủy yêu cầu sửa", true);
