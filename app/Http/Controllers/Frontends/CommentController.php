    ...
    public function storeCommentVideo(StoreDiscussionRequest $request)
    {
        ...
        if ($video) {
            ...
            if (isset($request->parentId)) {
                ...
                \App\Helper\Helper::addAlertCustomize($commentVideo->parent()->userRole()[0]->user, "Thảo luận của bạn có một trả lời", "Thảo luận của bạn ở bài giảng <a href='" . url('/') . "/learning-page/" . $video->unit->course->id . "/lecture/" . $video->id . "'>" . $video->name . "</a> vừa có một trả lời.", true);
            }else{
